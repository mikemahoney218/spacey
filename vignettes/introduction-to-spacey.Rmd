...
```r
automap(lat = 42.3601, lng = -71.0589, z = 0.3)
```
...
* First, `automap` looks at your coordinates (alongside other optional 
...
* Next, `automap` uses that information to query the 
...
* Finally, `automap` takes that data and creates a rayshader map using 
...
(Note that I'm cheating a little bit for this vignette -- in order to get 
properly sized graphics, the `get_heightmap` and `get_image_overlay` calls 
used to build these maps both used the argument `major.dim = 1200`, doubling 
...
is `automap`, which will let you customize most `rayshader` variables in a 
